...
class MessageSendingThread extends Thread {
	...
	public void run() {
		for(int i = 1 ; i<=1000 ; i++) {
			...
			sleep()메서드를 사용하게되면 실행 중 잠깐씩 블럭상태로
			전환되어 우선순위가 낮은 쓰레드도 가끔 실행될 수 있는
			싱황이 생기게 된다.
			 */
			try {
				sleep(1); //0.001초 동안 블럭상태로 전환
			}
...
public class Ex02Priorityplzcopy
{
	...
	public static void main(String[] args)
	{
		...
		1단계 실행
			...
			동일한 우선순위 5을 부여받게된다. 이때는 CPU가 알아서
			작업을 분배한다.
		 */
...
//		
		...
		/*
		3단계 실행
			:정수대신 static(정적) 상수를 사용하여 우선순위를 부여할 수도
			있다. 결과는 2단계와 동일하다.
		 */
