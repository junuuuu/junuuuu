...
public class Human
{
  ...
  void walking() {
	  ...
	  if(energy<0) {
		  ...
		  System.out.println("[walking]에너지가 1감소하였습니다.");
	  }
    }
  //'생각하다'를 표현
  void thinking() {
	  ...
	  if(energy<0) {
		  ...
		  System.out.println("[thinking]에너지가 1감소하였습니다.");
	  }
    }
  void eating() {
	  //에너지를 2증가시킨 후 10을 초과하면 10으로 고정한다.
	  energy += 2;
	  if(energy>10) {
		  energy = 10;
		  System.out.println("[eating]에너지는 10이 최대입니다."+"최대치입니다.");
		  
	  }
	  else {
		  System.out.println("[eating]에너지가 2증가하였습니다.");
	  }
    }
