...
class Person {
	...
	String job = "영화배우";
	/*
	멤버메서드 : 클래스에서 객체의 동작(행위)을 표현한다.
		클래스외부에서 호출할 때는 객체의 참조변수를 통해서
		호출한다.
		맴버변수는 멤버메서드 내에서 즉시 사용 가능하다.
	 */
	void eat() {
...
public class E01PersonMain
{
	...
	{
		/*
		 Person클래스를 통해 객체(인스터스)를 생성한다. 생성 시 할당된
		 주소값을 반환한다.
		 */
		
		Person person = new Person();
		
		//객체의 참조변수를 통해 멤버메서드를 호출할 수 있다.(점을 통해서 호출)
