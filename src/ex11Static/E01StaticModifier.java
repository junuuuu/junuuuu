...
/*
 ...
 -맴버변수 앞에 static 키워드가 붙으면 정적변수가 된다.
 -정적변수는 JVM(자바가상머신)에 의해 프로그램이 시작되는 시점에
 	초기화된다.
 -main() 메서드가 실행되기전에 이미 초기화되므로 main()메서드
 	실행 시 별도의 선언 없이 변수를 사용할 수 있다.
 	
 접근방법
 	-클래스내부: 변수명으로 접근가능(일반적인 멤버변수와 동일함)
 	-클래스외부: 참조변수를 통해 접근 or 클래스명을 통한 접근
 		둘다 가능하다. //단, 클래스명으로 접근할 것을 권장한다.
 */


class MyStatic {
	/*
	멤버변수 ] 인스턴스형 멤버변수(일반적인 멤버변수)는 반드시 
		클래스의 객체를 생성한 후 참조변수를 통해 접근해야한다.
	 */
	int instanceVar;
	/*
	...
	/*
	인스턴스형 멤버메서드] 인스턴스형 메소드에서는 모든 멤버를 사용할 수 있다.
		인스턴스형 변수, 정적 변수 둘다 접근 가능하다.
	 */
	void instanceMethod() {
		System.out.println("intstanceVar="+ instanceVar);
		System.out.println("staticVar="+staticVar);
		staticMethod();
	}
	/*
	정적 멤버메서드] 정적 메서드 안에서는 인스턴스형 멤버를 사용할 수 없다.
...
public class E01StaticModifier
{
	...
	public static void main(String[] args)
	{
	    ...
	    /*
	    정적변수도 일반적인 멤버변수처럼 참조변수를 통해 접근하는 것이
		...
		System.out.println("ms.instanceVar="+ms1.instanceVar);
		System.out.println("ms.instanceVar="+ms2.instanceVar);
		...
		/*
		정적 멤버메소드의 접근방법은 정적변수의 접근 방법과 동일하다.
		 */
		myStatic.instanceMethod();
		myStatic.staticMethod(); //권장사항이 아니므로 클래스명을 사용하는게 좋다
		
		//MyStatic.instanceMethod(); //=>에러발생. 참조변수를 통해서만 호출 할 수있다.
