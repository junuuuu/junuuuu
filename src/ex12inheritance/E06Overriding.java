...
/*
오버라이딩(Overriding)
	:클래스가 상속 관계에 있을 때 하위클래스에서 상위클래스의
	멤버메서드와 완전히 동일한 모양으로 재정의 하는 것을 말한다.
	오버로딩과 다른 점은 매개변수의 갯수, 반환타입 등이 완전히
	똑같은 형태로 재정의 한다는 것이다.
...
class BaseSpeaker extends Speaker {
	...
	public void showState() {
		/*
		super을 통해 부모클래스에 정의된 멤버메서드를 호출한다.
		특히, 오버라이딩된 메서드를 호출 할 때 super가 없으면
...
public class E06Overriding
{
	...
	public static void main(String[] args)
	{
		...
		baseSpeaker.showState();/*
		 	...
		 	showState()	메서드가 호출된다. 부모쪽의 메서드는 가려지게 된다.
	    */
		System.out.println();
