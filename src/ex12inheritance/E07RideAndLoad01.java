...
class C extends B {
	...
	정의도니 메서드와 매개변수의 갯수, 타입이 서로 다르다.
	 */
	public void loadMethod(double num) { 
		System.out.println("c의 loadMethod");
	}
	/*
	따라서 C클래스에 객체를 생성하면 오버라이딩된 메서드는
	모두 가려지게 되므로 C클래스에 정의된 메서드 하나만
	남게된다. 오버로딩된 메서드는 서로 다르므로 3개 모두
	존재하게된다.
	 */
}
public class E07RideAndLoad01
{
	...
	public static void main(String[] args)
	{
		...
		C refAddr1 = new C(); //가능
		C refAddr2 = refAddr1; //B타입 => C타입 : 가능
		C refAddr3 = refAddr1; //A타입 => C타입 : 가능
		/*
		참조변수 refId이 C객체를 참조하고 있으므로 마치 refId이 C타입이라고
