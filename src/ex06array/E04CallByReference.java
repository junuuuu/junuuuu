...
/*
 Call by reference(참조에 의한 호출)
  : 메소드를 호출할 때 참조값을 전달함으로써 서로 다른 지역에서도
  	동일한 메모리(힙영역)을 참조할 수 있게 하는 호출 방식이다.
  	메소드의 지역이 다르더라도 참조값을 통해 참조하게되므로 A영역에서의
  	값의 변경이 B영역에서도 그대로 참조할 수 있는 장점이 있다.
  	참조값만 전달하므로 프로그램의 퍼모먼스(성능)도 향상되고 메모리도
  	절약할 수 있다. 
 */
...
public class E04CallByReference
{
	...
	{    
		//배열변수를 생성한다.
		int[] arr = {100, 200};
		System.out.println("[메인메소드]-Swap이전 출력");
