...
public class E02StringMethod
{
	...
	/*
	시나리오] 해당 문자열이 이메일 형식인지 검사하는 프로그램을
	작서아시오.	
	 */
	...
	/*
	 9.indexOf() :문자열에서 특정 문자열의 시작인덱스를 반환한다.
	 ...
	 */      
		 ...
		 */
			  ...
			  juminNum = "120403-301245";
			  //주민번호에서 하이픈(-)의 위치를 찾아 1를 더한다.
	...
	/*
	 10. lastIndexOf() : 사용법은 indexOf()와 동일하나 문자열의 뒤에서부터
		찾는 것이 다르다.
	 */	
	...
	/*
	 11. replace : 특정 문자열을 찾아서 지정된 문자열로 변경한다.
	 	만약 찾는 문자열이 없다면 변경만 되지 않을뿐 에러가 발생하지는
	 	않는다.
	 */   System.out.println("### replace() ####");
		  System.out.println("J를 G로 변경하기");
		  System.out.println(str1.replace("J", "G")); //변화없음
		  System.out.println("java를 kosmo로 변경하기");
		  System.out.println(str1.replace("java", "KOSMO")); //KOSMO로 변경
	
	/*
   	12. split : 문자열을 구분자를 통해 분리해서 String 타입의 배열로
	...
	*/
	...
	/*
	 13. substring() : 시작인덱스와 끝인덱스 사이의 문자열을 잘라서
	 	...
	 	사용2 : substring(시작인덱스,끝인덱스)=> 인덱스에 해당하는 부분만 잘라서
	 		반환한다.
	 */		
	 ...
	 /*
	 14. codePointAt(인덱스) :문자열의 특정 인덱스의 문자를 찾은 후 
	 	아스키코드(유니코드)를 반환한다.
