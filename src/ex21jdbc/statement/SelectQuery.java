	...
	void execute()
	{
		try {
			...
			while(rs.next()) {
				...
				/*
				오라클의 data타입을 getData()로 추출하면 0000-00-00
				형태로 출력된다. 이 경우 data형 자료가 되므로 java.sql.Date
				클래스의 참조변수로 저장해야 한다.
				 */
				java.sql.Date regidate = rs.getDate("regidate");
				/*
				오라클의 data타입을 getString()으로 추출하면
				0000-00-00 00:00:00 형태로 시간까지 출력할 수 있다.
				만약 적당한 크기로 자르고 싶다면 substring()을 이용한다.
				 */
				String regidate2 = rs.getString("regidate");
				String regidate3 = rs.getString("regidate").substring(0,13);
				//쿼리문을 사용한 별칭을 그대로 쓸 수 있다.
